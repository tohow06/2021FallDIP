...
// Soft shrinkage
//--------------------------------
float dwt2::soft_shrink(float d,float T)
{
    float res;
    if(fabs(d)>T)
    {
...
// Hard shrinkage
//--------------------------------
float dwt2::hard_shrink(float d,float T)
{
    float res;
    if(fabs(d)>T)
    {
...
// Garrot shrinkage
//--------------------------------
float dwt2::Garrot_shrink(float d,float T)
{
    float res;
    if(fabs(d)>T)
    {
...
// Wavelet transform
//--------------------------------
void dwt2::cvHaarWavelet(Mat &src,Mat &dst,int NIter)
{
    float c,dh,dv,dd;
...
//--------------------------------
void dwt2::cvInvHaarWavelet(Mat &src,Mat &dst,int NIter, int SHRINKAGE_TYPE, float SHRINKAGE_T)
{
    float c,dh,dv,dd;
    assert( src.type() == CV_32FC1 );
    assert( dst.type() == CV_32FC1 );
...
int process(VideoCapture& capture)
{
    ...
    for (;;)
    {
        ...
        dwt2::cvHaarWavelet(Src,Dst,NIter);

        Dst.copyTo(Temp);

        dwt2::cvInvHaarWavelet(Temp,Filtered,NIter,GARROT,30);
